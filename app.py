...
# --- 8. 營收動能策略邏輯 (🛠️ /optimize: 修改為 YoY 近三月平均) ---
elif mode == "營收動能策略":
    st.sidebar.info("💡 偵測 `revenue_data/` 中海量資料。計算單月 YoY 近三月平均。")
            ...
            with st.spinner(f"正在分析最新檔案: {[os.path.basename(f) for f in recent_files]}"):
                for f in recent_files:
                    try:
                        ...
                        cur_col = '營業收入-當月營收'
                        last_col = '營業收入-去年同月營收' # 確保抓取去年同月數據
                        ...
                        if '公司代號' in t_df.columns and cur_col in t_df.columns and last_col in t_df.columns:
                            ...
                            # 步驟一：計算該月 YoY
                            t_df['yoy'] = (t_df[cur_col] - t_df[last_col]) / t_df[last_col]
