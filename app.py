...
# --- 8. 營收動能策略邏輯 (修復 KeyError 與整合) ---
elif mode == "營收動能策略":
    st.sidebar.info("💡 系統將讀取 revenue_data/ 內近三月 CSV 計算平均成長率 > 20% 個股。")
    if st.sidebar.button("📊 啟動營收動能分析"):
        folder = "revenue_data"
        all_files = sorted(glob.glob(os.path.join(folder, "*.csv")), reverse=True)
        
        if len(all_files) < 3:
            st.error("CSV 檔案不足三個月，無法計算。")
        else:
            recent_files = all_files[:3]
            month_dfs = []
            with st.spinner("正在讀取並合併營收數據..."):
                ...
                if len(month_dfs) == 3:
                    ...
                    merged = m1[['公司代號', '公司名稱', '營業收入-當月營收']].rename(columns={'營業收入-當月營收': 'rev1'})
                    merged = merged.merge(m2[['公司代號', '營業收入-當月營收']], on='公司代號').rename(columns={'營業收入-當月營收': 'rev2'})
                    merged = merged.merge(m3[['公司代號', '營業收入-當月營收']], on='公司代號').rename(columns={'營業收入-當月營收': 'rev3'})
