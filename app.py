...
# --- 8. 營收動能策略邏輯 (🛠️ /refactor: 雙目錄 + 檔名倒序) ---
    ...
    st.sidebar.info("💡 偵測 `TWSE` 與 `TPEX` 目錄檔名倒序近三月，計算平均【年增率】。")
    
    def process_folder(folder_path):
        if not os.path.exists(folder_path): return pd.DataFrame()
        # 檔名倒序排序 (e.g. 202603.csv 排第一)
        all_files = sorted(glob.glob(os.path.join(folder_path, "*.csv")), reverse=True)
        if len(all_files) < 3: return pd.DataFrame()
        ...
        for f in recent_files:
            try:
                ...
                if all(col in t_df.columns for col in [col_code, col_rev_now, col_rev_last]):
                    ...
                    month_dfs.append(t_df[[col_code, '公司名稱', 'yoy']])
            except: continue
            
        if len(month_dfs) == 3:
            m1, m2, m3 = [df.drop_duplicates('公司代號') for df in month_dfs]
            merged = m1.rename(columns={'yoy': 'yoy1'}).merge(
                m2[['公司代號', 'yoy']].rename(columns={'yoy': 'yoy2'}), on='公司代號').merge(
                m3[['公司代號', 'yoy']].rename(columns={'yoy': 'yoy3'}), on='公司代號')
            merged['avg_growth'] = (merged['yoy1'] + merged['yoy2'] + merged['yoy3']) / 3 * 100
            return merged
        return pd.DataFrame()

    if st.sidebar.button("📊 啟動營收動能分析"):
        with st.spinner("掃描雙市場數據中..."):
            df_twse = process_folder("revenue_data_TWSE")
            df_tpex = process_folder("revenue_data_TPEX")
            all_targets = pd.concat([df_twse, df_tpex], ignore_index=True)
            
            targets = all_targets[all_targets['avg_growth'] > 20].copy()
            if targets.empty:
                ...
                for _, row in targets.iterrows():
                    ...
                    if not df_h.empty:
                        ...
                        rev_results.append({
                            "市場": info["市場"], "代號": code, "名稱": row['公司名稱'], 
