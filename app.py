...
# --- 8. 營收動能策略邏輯 (🔥 重構後的邏輯) ---
    ...
    if st.sidebar.button("📊 啟動營收動能分析"):
        ...
        if not all_files:
            st.error("⚠️ 資料夾內找不到任何 CSV 檔案。")
        else:
            file_info = []
            for f in all_files:
                try:
                    # 讀取首行確認資料年月
                    temp_df = pd.read_csv(f, nrows=1, encoding='utf-8-sig')
                except:
                    temp_df = pd.read_csv(f, nrows=1, encoding='cp950')
                
                # 提取資料年月作為排序標準 (例如 113/3)
                if '資料年月' in temp_df.columns:
                    month_str = str(temp_df['資料年月'].iloc[0])
                    file_info.append((f, month_str))
            
            # 按月份字串倒序排列，取前三名
            file_info.sort(key=lambda x: x[1], reverse=True)
            recent_files = [x[0] for x in file_info[:3]]
            
            if len(recent_files) < 3:
                st.warning(f"⚠️ 資料年月完整的檔案不足 3 份 (目前僅找到 {len(recent_files)} 份)。")
            else:
                month_dfs = []
                with st.spinner(f"正在分析最新三月: {[os.path.basename(f) for f in recent_files]}"):
                    for f in recent_files:
                        try:
                            ...
                            if all(col in t_df.columns for col in needed):
                                ...
                                month_dfs.append(t_df[['公司代號', '公司名稱', 'YoY']].dropna())
                        except: continue

                if len(month_dfs) == 3:
                    m1, m2, m3 = month_dfs[0], month_dfs[1], month_dfs[2]
                    # 合併三個月的 YoY
                    merged = m1.rename(columns={'YoY': 'y1'})
                    merged = merged.merge(m2[['公司代號', 'YoY']].rename(columns={'YoY': 'y2'}), on='公司代號')
                    merged = merged.merge(m3[['公司代號', 'YoY']].rename(columns={'YoY': 'y3'}), on='公司代號')
                    
                    # 計算三月平均年增率
                    merged['avg_yoy'] = (merged['y1'] + merged['y2'] + merged['y3']) / 3 * 100
                    targets = merged[merged['avg_yoy'] > 20].copy()
                    
                    if targets.empty:
                        st.info("目前無符合平均年增率 > 20% 的個股。")
                    else:
                        rev_results = []
