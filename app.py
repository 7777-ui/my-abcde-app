...
# --- 0. 🚀 即時數據抓取函數 (🛠️ /optimize: 加入市場別識別邏輯) ---
...
# --- 3. 🛡️ 族群 CSV 讀取 (🛠️ /scaffold: 增加市場別快取欄位) ---
...
def get_market_env():
    ...
    for k, v in rt_indices.items():
        ...
        except: res[k] = {"燈號": "⚠️ 斷訊", "價格": 0.0, "帶寬": 0.0}
    return res
...
# --- 7. 姊布林 ABCDE (🛠️ /optimize: 表格加入市場欄位) ---
if mode == "姊布林 ABCDE":
    ...
    if st.sidebar.button("🚀 開始掃描戰情") and raw_input:
        ...
        for code in codes:
            ...
            df = get_historical_data(code + suffix)
            if df.empty: # 若快取判斷失效則自動補償
            ...
            if not df.empty and len(df) >= 20:
                ...
                # ... (此處保留原有的 ABCDE 邏輯計算) ...
                # 為了簡潔，假設計算後得到 res_tag, chg, vol_amt, bw, ratio
                # [邏輯計算部分省略，與原版一致]
                
                results.append({
                    "市場": info["市場"], "代號": code, "名稱": info["簡稱"], "策略": "分析中...", # 這裡應接妳的邏輯
                    "現價": p_curr, "漲幅%": f"{0.0:.1f}%", "成交值(億)": 0.0,
                    "產業排位": info["產業排位"], "族群細分": info["族群細分"]
                })
        st.session_state.scan_results = pd.DataFrame(results)

# --- 8. 營收動能策略 (🛠️ /optimize: 整合市場別) ---
elif mode == "營收動能策略":
    if st.sidebar.button("📊 啟動營收動能分析"):
        # ... (此處保留原有的營收分析邏輯) ...
        # [營收計算部分省略，與原版一致]
        rev_results = []
        # 假設 targets 是妳計算出的 DataFrame
        for _, row in targets.iterrows():
            ...
            if p_curr:
                rev_results.append({
                    ...
                    "三月均增%": f"{row['avg_growth']:.1f}%", "現價": p_curr,
                    "產業排位": info["產業排位"], "族群細分": info["族群細分"]
                })
