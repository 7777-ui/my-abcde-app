...
def calculate_mtm_logic(df, p_curr):
    ...
    proc = (current_mtm / p_10_days_ago) * 100 if p_10_days_ago != 0 else 0
    
    # 動能判定：MTM > 0 且 MTM > MTM_MA (代表動能轉強)
    is_strong = current_mtm > 0 and current_mtm > mtm_ma10
    return round(current_mtm, 2), round(proc, 2), is_strong
                ...
                for _, row in df_local.iterrows():
                    ...
                    if code.isdigit():
                        mapping[code] = {
                            ...
                            "產業": str(row.iloc[2]).strip() if len(row) > 2 else "-"
                        }
            except: pass
    return mapping
    ...
    for k, v, yf_k in [("上市", "TSE", "^TWII"), ("上櫃", "OTC", "^TWOII")]:
        try:
            ...
            if not df_h.empty and curr_p:
                ...
                c_list = df_h['Close'].dropna().iloc[-19:].tolist() + [curr_p]
...
def process_scanning(codes, is_auto=False):
    ...
    for idx, code in enumerate(codes):
        my_bar.progress((idx + 1) / total, text=f"正在分析: {code}")
        ...
        # 為了效能，自動掃描時先看昨天成交量，若太小直接跳過
        vol_yest_amt = (df['Volume'].iloc[-1] * p_yest) / 100000000
        if is_auto and vol_yest_amt < 2: continue # 成交值未達 2 億
        ...
        # 自動掃描時，漲幅不到 2% 的也跳過
        if is_auto and chg < 0.02: continue 
        ...
        res_tag = ""
        # 判斷是否符合姊布林 (量能門檻設 5 億)
        if p_curr > upper and slope_pos and vol_yest_amt >= 5:
        ...
        results.append({
            ...
            "策略": res_tag if res_tag else "⚪ 僅動能強",
            "現價": p_curr, "漲幅%": f"{chg*100:.1f}%", "成交值(億)": round(vol_yest_amt, 1),
            "帶寬%": f"{bw*100:.1f}%", "產業": info["產業"]
        })
...
raw_input = st.sidebar.text_area("1. 手動輸入股號", height=100, placeholder="例如: 2330 2317")
if st.sidebar.button("🚀 開始掃描手動清單"):
    codes = re.findall(r'\b\d{4,6}\b', raw_input)
    if codes:
        st.session_state.scan_results = pd.DataFrame(process_scanning(codes, is_auto=False))

st.sidebar.markdown("---")
st.sidebar.write("2. 自動化雷達")
if st.sidebar.button("📡 啟動全市場動能掃描"):
    all_codes = list(stock_info_map.keys())
    if not all_codes:
        st.error("找不到 CSV 資料，請確認 TWSE.csv 與 TPEX.csv 是否存在。")
    else:
        results = process_scanning(all_codes, is_auto=True)
        if results:
            st.session_state.scan_results = pd.DataFrame(results)
        else:
            st.warning("當前市場無符合『漲幅>2%、成交值>2億且動能轉強』之標的。")
