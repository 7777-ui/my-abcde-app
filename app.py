...
# --- 8. 營收動能策略邏輯 (🛠️ 重構修正) ---
    ...
    st.sidebar.info("💡 分析 `revenue_data/` 最新三月平均年增率 > 20% 之個股。")
            ...
            with st.spinner(f"正在分析檔案: {[os.path.basename(f) for f in recent_files]}"):
                for f in recent_files:
                    try:
                        ...
                        # 檢查必要欄位是否存在
                        if all(col in t_df.columns for col in cols_needed):
                            # 資料清理
                            ...
                            # 計算該月單月年增率 (YoY)
                            t_df['yoy'] = (t_df['營業收入-當月營收'] - t_df['營業收入-去年當月營營收']) / t_df['營業收入-去年當月營收'] * 100
                ...
                if len(month_dfs) == 3:
                    ...
                    m2 = month_dfs[1][['公司代號', 'yoy']].rename(columns={'yoy': 'yoy2'})
                    m3 = month_dfs[2][['公司代號', 'yoy']].rename(columns={'yoy': 'yoy3'})
