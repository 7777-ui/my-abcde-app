...
# --- 5. 側邊欄與模式控制 ---
st.sidebar.title("🛠️ 戰情總控面板")
main_mode = st.sidebar.radio("切換掃描模式", ["🏹 姊布林 ABCDE", "📈 營收動能掃描"])

# =================================================================
# BLOCK A：🏹 姊布林策略區 (完整保留原始邏輯，不作變動)
# =================================================================
        ...
        with st.spinner("分析中..."):
            for code in codes:
                ...
                if not df.empty and len(df) >= 20:
                    ...
                    close_20 = history_for_ma + [p_curr]; m20_now = sum(close_20) / 20; std_now = pd.Series(close_20).std()
                    ...
                    results.append({
                        "代號": code, "名稱": info["簡稱"], "策略": res_tag, "現價": p_curr, "漲幅%": f"{chg*100:.1f}%", "成交值(億)": round(vol_amt, 1),
                        "個股帶寬%": f"{bw*100:.2f}%", "比值": round(ratio, 2), "產業排位": info["產業排位"], "2026指標": info["實力指標"], "族群細分": info["族群細分"]
                    })
...
# =================================================================
# BLOCK B：📈 營收動能策略區 (依據需求獨立建立)
# =================================================================
elif main_mode == "📈 營收動能掃描":
    st.markdown("### 📈 營收動能策略 - 近三月平均年增率分析")
    ...
    def run_revenue_analysis():
        ...
        # 步驟 1 & 2: 抓取最新 3 個月 CSV
        twse_files = sorted(glob.glob(os.path.join(path, "TWSE_*.csv")), reverse=True)[:3]
        tpex_files = sorted(glob.glob(os.path.join(path, "TPEX_*.csv")), reverse=True)[:3]
        
        def process_market(files, label):
            ...
            for f in files:
                ...
                df = df[['資料年月', '公司代號', '公司名稱', '營業收入-去年同月增減(%)']].copy()
                df['公司代號'] = df['公司代號'].astype(str).str.strip()
                df['營業收入-去年同月增減(%)'] = pd.to_numeric(df['營業收入-去年同月增減(%)'], errors='coerce').fillna(0)
                temp_list.append(df)
            
            # 合併並計算平均
            combined = pd.concat(temp_list)
            result = combined.groupby(['公司代號', '公司名稱'])['營業收入-去年同月增減(%)'].sum().reset_index()
            result['近三月平均年增%'] = result['營業收入-去年同月增減(%)'] / 3
            result['市場別'] = label
            return result[result['近三月平均年增%'] > 20]

        twse_final = process_market(twse_files, "上市")
        tpex_final = process_market(tpex_files, "上櫃")
        return pd.concat([twse_final, tpex_final])

    if st.sidebar.button("📊 執行營收動能分析"):
        with st.spinner("營收數據與即時行情同步中..."):
            momentum_df = run_revenue_analysis()
            final_report = []
            ...
            for _, row in momentum_df.iterrows():
                ...
                # 7. 現價、漲幅、成交值 (同步姊布林抓取邏輯)
                p_curr = get_realtime_price(code)
                if not p_curr: continue
                
                df_h = get_historical_data(f"{code}.TW" if m_type=="上市" else f"{code}.TWO")
                chg_str = "-"; vol_amt = 0
                if not df_h.empty:
                    if isinstance(df_h.columns, pd.MultiIndex): df_h.columns = df_h.columns.get_level_values(0)
                ...
                # 7-1. 對齊產業排位與族群細分 (去根目錄 CSV 找)
                info = stock_info_map.get(code, {"產業排位": "-", "族群細分": "-"})
                
                final_report.append({
                    "市場別": m_type, "代號": code, "名稱": name, "近三月平均年增%": round(avg_yoy, 2),
                    "現價": p_curr, "漲幅%": chg_str, "成交值(億)": round(vol_amt, 1),
                    "產業排位": info["產業排位"], "族群細分": info["族群細分"]
                })
            
            if final_report: st.session_state.revenue_results = pd.DataFrame(final_report)
            else: st.warning("未偵測到符合 >20% 門檻之標的。")

    if st.session_state.revenue_results is not None:
        st.info("💡 營收動能模式：顯示近三月平均營收年增率 > 20% 之個股。")
        # 4. 提供排序功能
        st.dataframe(st.session_state.revenue_results.sort_values(by="近三月平均年增%", ascending=False), use_container_width=True, hide_index=True)
