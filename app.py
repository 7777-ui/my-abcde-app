...
# --- 8. 營收動能策略邏輯 (修正算法) ---
    ...
    st.sidebar.info("💡 提取最新三個月 YoY 年增率並計算平均動能。")
        ...
        if len(all_files) < 3:
            ...
            with st.spinner("讀取海量營收數據中..."):
                ...
                if len(month_dfs) == 3:
                    ...
                    m1 = m1.drop_duplicates('公司代號').rename(columns={target_col: 'yoy1'})
                    m2 = m2.drop_duplicates('公司代號').rename(columns={target_col: 'yoy2'})
                    m3 = m3.drop_duplicates('公司代號').rename(columns={target_col: 'yoy3'})
                    
                    # 整合三個月 YoY
                    merged = m1.merge(m2[['公司代號', 'yoy2']], on='公司代號').merge(m3[['公司代號', 'yoy3']], on='公司代號')
                    
                    # 算法優化：計算三個月 YoY 的算術平均值
